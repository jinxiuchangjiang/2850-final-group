...
class UserController(
    ...
    /** Periodic presence ping — call every 30s from the frontend to mark this user as online. */
    /**
     * Heartbeat — call every 30s.
     * Body: { "status": "IDLE" } or { "status": "IN_GAME" }
     */
     ...
     *   1. liked_games, play_history, friendships
     *   2. room_players (as a participant)
     *   3. room_players (in rooms this user hosted) + those rooms
     *   4. user row
     */
    @DeleteMapping("/me")
    @Transactional
...
class GameController(private val gameRepo: GameRepository) {
    ...
    fun Game.toDto() = GameDto(id, title, author, description, rules, coverImageUrl,
        gameFileUrl, durationMinutes, minimumAge, getTagList())
}
...
class AdminGameController(
    ...
    fun create(@RequestBody req: CreateGameRequest): ResponseEntity<ApiResponse<GameDto>> {
        val g = gameRepo.save(Game(title = req.title, author = req.author,
            description = req.description, rules = req.rules,
            durationMinutes = req.durationMinutes, minimumAge = req.minimumAge)
            .also { it.setTagList(req.tags) })
    ...
    fun update(@PathVariable id: Long, @RequestBody req: UpdateGameRequest): ResponseEntity<ApiResponse<GameDto>> {
        ...
        req.durationMinutes?.let { g.durationMinutes = it }; req.minimumAge?.let { g.minimumAge = it }
        req.tags?.let { g.setTagList(it) }; g.updatedAt = LocalDateTime.now()
    ...
    // PDF upload endpoint intentionally removed — see requirements
...
// ── ADMIN USERS  (view + ban/unban only; delete is intentionally disabled) ───
