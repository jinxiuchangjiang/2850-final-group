...
async function createRoom() {
    ...
    const gameMax = currentGameData?.maxPlayers ?? 2;
    const res = await api('POST','/api/rooms',{gameId:currentGameId,roomName:name,privacy,password:pw,maxPlayers:gameMax});
...
async function loadAdminSec(sec) {
    if (sec === 'adash')  await loadAdminDash();
    if (sec === 'agames') await loadAdminGames();
    if (sec === 'ausers') await loadAdminUsers();
    if (sec === 'acats')  await loadAdminTags();
}

async function loadAdminDash() {
    const [sr, gr, lr] = await Promise.all([
        api('GET', '/api/admin/stats/summary'),
        api('GET', '/api/games'),
        api('GET', '/api/admin/stats/user-growth')
    ]);
    const s = sr.data || {};
    document.getElementById('asgames').textContent  = s.totalGames  ?? '—';
    document.getElementById('asusers').textContent  = s.totalUsers  ?? '—';
...
function renderPie(tc) {
    ...
    adminCharts.pie = new Chart(ctx, {
        type: 'doughnut',
        data: {
            labels,
            datasets: [{ data, backgroundColor: PIE_COLORS, borderWidth: 2, borderColor: '#fff' }]
        },
        options: {
            responsive: false,
    ...
    document.getElementById('pileg').innerHTML = labels.map((l, i) =>
        `<div class="pleg-item">
            <div class="pleg-dot" style="background:${PIE_COLORS[i % PIE_COLORS.length]}"></div>
            <span>${l} (${data[i]})</span>
        </div>`
    ).join('');
}

function renderLine(growth) {
    const ctx = document.getElementById('linechart');
    if (!ctx) return;
    if (adminCharts.line) adminCharts.line.destroy();

    adminCharts.line = new Chart(ctx, {
        type: 'line',
        data: {
            ...
            datasets: [{
                ...
                borderColor: '#5aaa38',
                backgroundColor: 'rgba(90,170,56,.12)',
                fill: true, tension: .35,
                pointRadius: 3, pointBackgroundColor: '#5aaa38'
            }]
        },
        options: {
            responsive: true,
...
async function loadAdminGames() {
    ...
    _ag = gr.data || [];
    allTags = tr.data || [];
    renderAdminGames(_ag);
}

function renderAdminGames(games) {
    document.getElementById('agtb').innerHTML = games.map(g => `
        <tr>
            <td>${g.coverImageUrl
                ? `<img class="img-thumb" src="${g.coverImageUrl}" alt="">`
            ...
            <td>
                ...
                ${g.gameFileUrl
                    ...
                    : '<span class="file-badge built-in">built-in</span>'
                }
            </td>
            <td>${g.author}</td>
            <td>${g.tags.map(t => `<span class="gtag">${t}</span>`).join(' ')}</td>
            <td>
                <button class="btn-green bsm" onclick="openEditGame(${g.id})">Edit</button>
                <button class="btn-red bsm" style="margin-left:4px" onclick="delGame(${g.id})">Delete</button>
            </td>
        </tr>`
    ).join('');
}
...
function asr_games(q) {
    ...
    renderAdminGames(_ag.filter(g =>
        ...
        g.author.toLowerCase().includes(lq)
    ));
}
...
function openAddGameModal() {
    ...
    document.getElementById('agrules').innerHTML = '';
    document.getElementById('agfn').textContent  = '';
    document.getElementById('agimgprev').innerHTML = '<img src="icons/image.svg" style="width:36px;opacity:.4" alt=""/>';
    renderTagSel('ag', agTags);
    openMod('m-addgame');
}

async function addGame() {
    const title = v('agtitle');
    ...
    const body = {
        ...
        rules:           document.getElementById('agrules').innerHTML,
        durationMinutes: parseInt(document.getElementById('agdur').value)  || 60,
        minimumAge:      parseInt(document.getElementById('agage').value)  || 6,
        maxPlayers:      parseInt(document.getElementById('agplayers').value) || 2,
        tags:            agTags
    };

    const res = await api('POST', '/api/admin/games', body);
    if (!res.success) { toast(res.message || 'Failed', 'err'); return; }

    const gid = res.data.id;
    const cf = document.getElementById('agimg').files[0];
    if (cf) {
        const fd = new FormData();
        fd.append('file', cf);
        await api('POST', `/api/admin/games/${gid}/cover`, fd, true);
    }
    const gf = document.getElementById('agfile').files[0];
    if (gf) {
        const fd = new FormData();
        fd.append('file', gf);
        await api('POST', `/api/admin/games/${gid}/game-file`, fd, true);
    }
...
async function openEditGame(id) {
    ...
    document.getElementById('egidx').value    = id;
    document.getElementById('egtitle').value  = g.title;
    document.getElementById('egauth').value   = g.author;
    document.getElementById('egdur').value    = g.durationMinutes;
    document.getElementById('egage').value    = g.minimumAge;
    document.getElementById('egplayers').value = g.maxPlayers ?? 2;
    document.getElementById('egdesc').value   = g.description;
    document.getElementById('egrules').innerHTML = g.rules || '';

    const prev = document.getElementById('egimgprev');
    prev.innerHTML = g.coverImageUrl
        ? `<img class="cover-prev" src="${g.coverImageUrl}" alt=""/>`
        : '<img src="icons/image.svg" style="width:36px;opacity:.4" alt=""/>';
...
async function saveEditGame() {
    ...
    const body = {
        ...
        rules:           document.getElementById('egrules').innerHTML,
        durationMinutes: parseInt(document.getElementById('egdur').value)  || 60,
        minimumAge:      parseInt(document.getElementById('egage').value)  || 6,
        maxPlayers:      parseInt(document.getElementById('egplayers').value) || 2,
        tags:            egTags
    };

    const res = await api('PUT', `/api/admin/games/${id}`, body);
    if (!res.success) { toast(res.message || 'Failed', 'err'); return; }

    const cf = document.getElementById('egimg').files[0];
    if (cf) {
        const fd = new FormData();
        fd.append('file', cf);
        await api('POST', `/api/admin/games/${id}/cover`, fd, true);
    }
    const gf = document.getElementById('egfile').files[0];
    if (gf) {
        const fd = new FormData();
        fd.append('file', gf);
        await api('POST', `/api/admin/games/${id}/game-file`, fd, true);
    }
...
function renderTagSel(prefix, selected) {
    document.getElementById(prefix + '-tag-sel').innerHTML = allTags.map(t =>
        `<span class="tsel-pill ${selected.includes(t.name) ? 'on' : ''}"
               onclick="toggleTag('${prefix}','${t.name}')">${t.name}</span>`
    ).join('');
    document.getElementById(prefix + '-selected-tags').innerHTML = selected.map(t =>
        `<span class="stag">${t}<span class="rx" onclick="removeTag('${prefix}','${t}')">x</span></span>`
    ).join('');
}

function toggleTag(p, name) {
    const a = p === 'ag' ? agTags : egTags;
    a.includes(name) ? a.splice(a.indexOf(name), 1) : a.push(name);
    renderTagSel(p, a);
}

function removeTag(p, name) {
    const a = p === 'ag' ? agTags : egTags;
...
async function loadAdminUsers() {
    ...
    _au = res.data || [];
    renderAdminUsers(_au);
}

function renderAdminUsers(users) {
    document.getElementById('autb').innerHTML = users.map(u => `
        <tr>
            <td>
                <strong>${u.fullName}</strong>
                <div style="font-size:.75rem;color:#888">@${u.username}</div>
            </td>
            <td><code style="font-size:.77rem">${u.uid}</code></td>
            <td>${u.email}</td>
            <td>${u.registeredAt}</td>
            <td><span class="ustatus ${u.status === 'ACTIVE' ? 'active' : 'banned'}">${u.status}</span></td>
            <td>
                <span class="online-dot ${u.isOnline ? 'on' : 'off'}"></span>
                ${u.isOnline ? 'Online' : 'Offline'}
            </td>
            <td>${u.status === 'ACTIVE'
                ? `<button class="btn-red bsm" onclick="banUser(${u.id})">Ban</button>`
                : `<button class="btn-green bsm" onclick="unbanUser(${u.id})">Unban</button>`
            }</td>
        </tr>`
    ).join('');
}

function asr_users(q) {
    const lq = q.toLowerCase();
    renderAdminUsers(_au.filter(u =>
...
async function loadAdminTags() {
    ...
    allTags = res.data || [];
    document.getElementById('acattb').innerHTML = allTags.map(t => `
        <tr>
            <td><strong>${t.name}</strong></td>
            <td>${t.description}</td>
            <td>
                <button class="btn-green bsm" onclick="openEditTag(${t.id},\`${t.name}\`,\`${t.description}\`)">Edit</button>
                <button class="btn-red bsm" style="margin-left:4px" onclick="delTag(${t.id})">Delete</button>
            </td>
        </tr>`
    ).join('');
}
...
function openAddTagModal() {
    ...
    document.getElementById('cat-modal-title').textContent = 'Add New Tag';
    openMod('m-addcat');
}
...
function openEditTag(id, name, desc) {
    ...
    document.getElementById('ncdesc').value          = desc;
    document.getElementById('cat-modal-title').textContent = 'Edit Tag';
    openMod('m-addcat');
}
...
async function saveCat() {
    ...
    const res = id
        ? await api('PUT',  `/api/tags/${id}`, { name, description: desc })
        : await api('POST', '/api/tags',        { name, description: desc });

    if (!res.success) { toast(res.message || 'Failed', 'err'); return; }
    closeMod('m-addcat');
    toast(id ? 'Tag updated!' : 'Tag added!');
    await loadAdminTags();
}
...
async function adminChPw() {
    ...
    const n2 = v('acpnew2');
    if (n !== n2) {
        setE('apwerr', 'Passwords do not match.');
        document.getElementById('apwerr').style.display = '';
        return;
    }
    document.getElementById('apwerr').style.display = 'none';
    const res = await api('POST', '/api/users/me/password',
        { oldPassword: o, newPassword: n, confirmPassword: n2 });
    if (res.success) { closeMod('m-admin-chpw'); toast('Password changed!'); }
    else toast(res.message || 'Failed', 'err');
}
...
function prevCover(input, prevId) {
    ...
    r.onload = e => {
        document.getElementById(prevId).innerHTML = `<img class="cover-prev" src="${e.target.result}" alt=""/>`;
    };
    r.readAsDataURL(file);
}

function fnChosen(input, labelId) {
    const el = document.getElementById(labelId);
    if (el) el.textContent = input.files[0]?.name || '';
}

function rtc(edId, cmd, val) {
